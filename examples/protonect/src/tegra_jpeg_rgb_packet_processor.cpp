...
int fill_input_buffer(j_decompress_ptr dinfo)
{
  dinfo->src->bytes_in_buffer = (size_t)dinfo->client_data;
  return 1;
}
...
public:
  ...
  void decompress(unsigned char *buf, size_t len)
  {
    ...
    dinfo.client_data = (void*)len;
    dinfo.src->bytes_in_buffer = 0;
    dinfo.src->fill_input_buffer = fill_input_buffer;
