...
void RgbPacketStreamParser::onDataReceived(unsigned char* buffer, size_t length)
{
  ...
  if(length > 0)
  {
    ...
    if (footer->magic_header == 0x39393939 && footer->magic_footer == 0x42424242)
    {
      ...
      {
        std::cerr << "[RgbPacketStreamParser::handleNewData] not enough space for packet filler!" << std::endl;
        fb.length = 0;
        return;
      }
      ...
      if (jpeg_length == 0)
      {
        std::cerr << "[RgbPacketStreamParser::handleNewData] no JPEG detected!" << std::endl;
        fb.length = 0;
        return;
      }
